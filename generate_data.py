...
def generate_fake_data(num_records=5000, start_index=0):
    ...
    sources = ['Canon EOS', 'Nikon D', 'iPhone', 'Samsung Galaxy', 'Sony Alpha', 'Fujifilm X', 'Olympus OM', 'Scanned Photo']
    locations = [
    ...
    user_tags_pool = [
        'Landscape', 'Portrait', 'Street', 'Nature', 'Urban', 'Sunset', 'Sunrise', 'Beach', 'Mountain',
        'Forest', 'City', 'Architecture', 'Food', 'Travel', 'Family', 'Wedding', 'Event', 'Macro',
        'Wildlife', 'Sports', 'Concert', 'Festival', 'Holiday', 'Vacation', 'Work', 'Art', 'Abstract',
        'Black and White', 'Vintage', 'HDR', 'Panorama', 'Night', 'Golden Hour', 'Blue Hour'
    ]
    
    hockey_teams = ['Flames', 'Oilers', 'Canucks', 'Leafs', 'Canadiens', 'Senators', 'Jets', 'Rangers', 'Bruins', 'Blackhawks']
    ...
    for i in range(num_records):
        ...
        resolutions = [
            (1920, 1080), (3840, 2160), (4000, 3000), (6000, 4000), 
            (5472, 3648), (4032, 3024), (3264, 2448), (2048, 1536)
        ]
        ...
        record = {
            ...
            'Image Y': height,
            'DPI': dpi,
            '(Center) Coordinate': coordinate if random.random() > 0.1 else '',  # 90% have coordinates
            'Favorite': 'Yes' if random.random() < 0.15 else '',  # 15% are favorites
            'Continent': continent if random.random() > 0.05 else '',  # 95% have continent
            'Bit color': bit_color if random.random() > 0.2 else '',  # 80% have bit color
            'Alpha': 'Y' if random.random() < 0.3 else '',  # 30% have alpha
            'Hockey Team': random.choice(hockey_teams) if random.random() < 0.1 else '',  # 10% have hockey team
            'User Tags': user_tags if random.random() > 0.1 else ''  # 90% have user tags
        }
...
def write_csv(records, filename='image_library.csv', append=False):
    fieldnames = [
        'Filename', 'Type', 'Image Size (MB)', 'Image X', 'Image Y', 'DPI',
        '(Center) Coordinate', 'Favorite', 'Continent', 'Bit color', 'Alpha',
        'Hockey Team', 'User Tags'
    ]
...
if __name__ == "__main__":
    ...
    print(f"Generated {len(records)} records in image_library.csv (total: {total_lines} records)")